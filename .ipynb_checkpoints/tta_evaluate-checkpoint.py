...
def evaluate_tta(source, target):
    ...
    for num_leads in leads_cfg:
        ...
        print('1. Source method...')
        evaluate_test(model, iterator_train, 'source')
        
        ### norm method : adjusting batch normalization on test batch
        # norm model
        print('2. Norm method...')
        norm_model = norm.Norm(model)
        evluate_test(norm_model, iterator_train, 'norm')
        ...
        evaluate_test(tented_model, iterator_train, 'tent-episodic')

        ### tent method (online) : tent method online adaptation
        print('4. Tent - online method...')
        tent_episodic = False
        tented_model = tent.Tent(model, optimizer, episodic=tent_episodic)
        evaluate_test(tented_model, iterator_train, 'tent-online')
        
    
# Inference
def evaluate_test(model, iterator_train, method): 
    ...
    if not os.path.exists('results'):
        os.mkdir('results')
    np.savez(f'results/{method}_{source}-{target}_{num_leads}_leads.npz', scalar = np.array(scalars), binary = np.array(binaries), labels = np.array(true_labels))

    # evaluate and save results
    evaluate_score(np.array(scalars), np.array(binaries), np.array(true_labels), method)
    
    
# Evaluate the performance and save the results
def evaluate_score(scalar_outputs, binary_outputs, labels, method):
    ...
    with open(f"results/{method}_{source}-{target}_{num_leads}_leads_{challenge_metric:.3f}.txt", 'w') as f:
